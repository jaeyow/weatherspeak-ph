...
#### 1. Chart description prompt deduplication (`ocr_marker.py`)

The system and user prompts for `_describe_chart` had significant duplication — the four analysis steps appeared in both prompts, and the output format was defined twice. Refactored to a single source of truth: system prompt owns all methodology (steps + output format), user prompt is now just the bulletin text block plus a one-line trigger. Approximately 30% fewer tokens per inference call.

Also added a `LOCATION RULE` to the Outlook section: the model must use distance + direction + landmark descriptions (e.g. "270 km Northwest of Pag-asa Island") rather than raw degree coordinates. Coordinates are still used internally in the Analysis section for spatial reasoning.

#### 2. Remove dead TL/CEB entries from `_RADIO_PROMPTS` (`scripts.py`)

`_RADIO_PROMPTS["tl"]` and `_RADIO_PROMPTS["ceb"]` were dead code — TL and CEB radio scripts are generated by translating from English via `_translate_radio_script`, not by calling `_generate_radio_script` directly. The system prompts were only still alive because `_TRANSLATE_PROMPTS` referenced them. Inlined the system prompts directly into `_TRANSLATE_PROMPTS` and removed the `tl`/`ceb` entries from `_RADIO_PROMPTS` entirely.

### Files changed

| File | Change |
|---|---|
| `modal_etl/core/ocr_marker.py` | Deduplicate system + user prompts; add LOCATION RULE to Outlook |
| `modal_etl/core/scripts.py` | Remove dead `_RADIO_PROMPTS["tl"]` and `["ceb"]`; inline into `_TRANSLATE_PROMPTS` |
...
**Status:** Open 🔄  
**Link:** https://github.com/jaeyow/weatherspeak-ph/pull/26
