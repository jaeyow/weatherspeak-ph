"""Pure-Python script generation logic for WeatherSpeak PH Step 2.
...
_RADIO_PROMPTS = {
    "en": {
        ...
        "user": (
            ...
            "{bulletin_data}\n\n"
            "STEP 1 — Extract these exact values from the bulletin text above (do not guess):\n"
            "- Storm name: (copy exact spelling from bulletin)\n"
            "- Category: (e.g. Typhoon, Severe Tropical Storm)\n"
            "- Max sustained winds: ___ km/h\n"
            "- Gusts: ___ km/h\n"
            "- Current position: (copy exact reference landmark)\n"
            "- Movement: (direction + speed)\n"
            "- Wind signals in effect: (list signal levels and areas, or 'none')\n"
            "- Gale warnings: (copy from bulletin)\n"
            "- Next update: (copy time/date from bulletin)\n\n"
            "STEP 2 — Write the full 350–400 word English announcement using ONLY the values you "
            "extracted above. Do not use any value you did not find in the bulletin. "
            "Include: storm name and category, wind speed and gusts in km/h, exact current position, "
            "whether it is strengthening or weakening, storm track paragraph (from the Storm Track Map "
            "section) covering forecast direction and which landmasses it passes near, "
            "Signal levels with affected areas grouped by region, gale warnings with specific sea areas "
            "and wave heights, what people must do, and when the next update is. "
            "No headings. No markdown. ENGLISH ONLY. Do not stop before 350 words."
        ),
    },
    "tl": {
        "system": (
